        ...
        <Presence present={open}>
          {({ present }) => (
            <RemoveScroll
              ...
              data-state={state}
              className={cn(
                'fixed text-[15px] flex flex-col py-2 rounded-e-2xl border-e end-0 inset-y-0 w-[85%] max-w-[380px] z-40 bg-fd-background data-[state=open]:animate-fd-sidebar-in data-[state=closed]:animate-fd-sidebar-out',
                !present && 'invisible',
                props.className,
              )}
            >
...
export function SidebarHeader(props: ComponentProps<'div'>) {
  return (
    <div
      {...props}
      className={cn('flex flex-col gap-3 p-4 pb-2 pr-6', props.className)}
    >
      {props.children}
    </div>
  );
